...
- [ ] `P2` `feature` `@me` Decide whether Lab Hub should start at login (LaunchAgent) or stay manual
- [ ] `P2` `testing` `@ai` Extend `--selftest` to launch every registered app, not just a sibling sample
- [ ] `P3` `design` `@ai` Apps tab: group by category rather than one flat list, now that there are twelve projects
